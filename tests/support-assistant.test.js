...
describe('support assistant replies', () => {
  ...
  test('explains bidon as 20 liter product with prices', () => {
    ...
    expect(text).toContain('recarga con bidon vacio cuesta S/ 65');
    expect(text).toContain('envase nuevo retornable cuesta S/ 85');
    expect(text).toContain('menor precio por litro');
    expect(text).toContain('S/ 3.25 por litro');
  ...
  test('registers bidon recarga as 20 liter order and accepts cash', () => {
    ...
    expect(first).toContain('S/ 65.00');
