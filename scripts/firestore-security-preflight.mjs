...
function localChecks(businessId) {
  ...
  return [
    ...
    check(index.includes('const CATALOGO_DEFAULT = [') && index.includes('function crearCatalogoBase(data = {})'), 'La app define el catalogo base del negocio'),
    check(index.includes('window.CATALOGO = crearCatalogoBase('), 'La app inicializa CATALOGO en memoria'),
    check(index.includes('function aplicarNegocioPublico(data = {})') && index.includes('function negocioRedesPublicas()'), 'La app aplica la ficha publica del negocio en tiempo real'),
    check(index.includes('function ownerGestionNegocio()') && index.includes('Datos del negocio'), 'El panel del dueno permite editar la ficha publica del negocio'),
    ...
    check(index.includes("businessId: CATALOGO_BUSINESS_ID") && index.includes("business_id: CATALOGO_BUSINESS_ID_CANONICAL") && index.includes("window.db.collection('catalogo').doc(nuevoId).set(fbData)"), 'Catalogo publico guarda businessId en cada producto'),
